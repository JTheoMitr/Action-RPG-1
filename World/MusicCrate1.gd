...
const AlbumPickup = preload("res://World/AlbumPickUp1.tscn")
...
func _on_Hurtbox_area_entered(area):
	...
	var randomDrop = random_drop_generator(["record_1",])
	var album_pickup = AlbumPickup.instance()
	get_parent().call_deferred("add_child", album_pickup)
	call_deferred("set_pos", album_pickup)
